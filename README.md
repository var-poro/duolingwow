...
- **World map tooltips** : hover a map label to see available abbreviations for that zone or instance.
- **Commands** : `/dl help`, `/dl status`, `/dl lang`, `/dl extension`, `/dl type`, `/dl map` to manage filters and map tooltips.
...
- **World map** : toggle map abbreviations on hover.
...
| `/dl status` | Print currently enabled languages, extensions, zone types, and map abbreviations. |
...
**World map** — control map tooltip abbreviations:

| Command | Description |
|---------|-------------|
| `/dl map enable` | Enable map tooltip abbreviations. |
| `/dl map disable` | Disable map tooltip abbreviations. |
